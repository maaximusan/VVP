...
class TheGem_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	private function get_item_data($item_id) {
		...
		$this->thegem_mobile_collapsible = get_post_meta( $item_id, '_menu_item_thegem_mobile_collapsible', true );
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ($depth == 0 && $this->tree_megamenu_root['enable'] && $this->tree_megamenu_root['source'] !== 'template') {
			if (isset($styles['padding-left']) && intval($styles['padding-left']) == 0) {
				$classes[] = 'megamenu-empty-left';
			}

			if (isset($styles['padding-right']) && intval($styles['padding-right']) == 0) {
				$classes[] = 'megamenu-empty-right';
			}

			if (isset($styles['padding-top']) && intval($styles['padding-top']) == 0) {
				$classes[] = 'megamenu-empty-top';
			}

			if (isset($styles['padding-bottom']) && intval($styles['padding-bottom']) == 0) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ($this->thegem_mobile_collapsible)
		...
		if (true) {
			...
			if (stripos($class_names, 'menu-item-parent') !== false || stripos($class_names, 'megamenu-template-enable') !== false) {
			...
			if (stripos($class_names, 'menu-item-parent') !== false) {
				$item_output .= '<span class="menu-item-parent-toggle" tabindex="0"></span>';
			}
