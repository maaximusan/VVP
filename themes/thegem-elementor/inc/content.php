			...
			<div class="related-posts-carousel">
				<?php while ($related_query->have_posts()) : $related_query->the_post(); ?>
					<div class="related-element">
						<a href="<?php echo esc_url(get_permalink()); ?>" aria-label="<?php echo esc_attr(get_the_title()); ?>"><?php thegem_post_thumbnail('thegem-post-thumb', true, '', array('srcset' => array('1x' => 'thegem-post-thumb-small', '2x' => 'thegem-post-thumb-large'))); ?></a>
    ...
    <div id="comment-<?= comment_ID(); ?>" <?= comment_class(empty($args['has_children']) ? '' : 'parent'); ?>>
        <div class="post-comment__wrap">
			<?php if(!empty($args['avatar'])): ?>
                <div class="post-comment__avatar"><?= get_avatar($comment, $args['avatar_size'], '', get_comment_author()); ?></div>
			<?php endif; ?>
...
<form role="search" method="get" id="top-area-searchform" class="searchform" action="<?php echo esc_url(home_url('/')); ?>">
	<div>
		<input type="text" value="<?php echo get_search_query(); ?>" name="s" id="top-area-s" aria-label="<?php esc_html_e('Search', 'thegem'); ?>" />
		<button type="submit" id="top-area-searchsubmit" value="<?php echo esc_attr_x('Search', 'submit button', 'thegem'); ?>" aria-label="<?php esc_html_e('Search', 'thegem'); ?>"></button>
	</div>
</form>
<?php
}
	...
	<?php if ($show): ?>
		<div class="post-author-block rounded-corners clearfix">
			<?php if ( get_the_author_meta('url', $user_id) ) : ?>
				<a href="<?php echo esc_url( get_the_author_meta('url', $user_id) ); ?>" class="post-author-avatar"><?php echo get_avatar( $user_id, 100, '', get_the_author() ); ?></a>
			<?php else : ?>
				<div class="post-author-avatar"><?php echo get_avatar( $user_id, 100, '', get_the_author() ); ?></div>
			<?php endif; ?>
...
function thegem_get_search_form($form) {
	...
	$form = '<form role="search" method="get" id="searchform" class="searchform" action="' . esc_url(home_url('/')) . '">
				<div>
					<input type="text" value="' . get_search_query() . '" name="s" id="s" aria-label="' . esc_html__('Search', 'thegem') .'" />'.$product_search.'
					 <button class="gem-button" type="submit" id="searchsubmit" value="' . esc_attr_x('Search', 'submit button', 'thegem') . '" aria-label="' . esc_html__('Search', 'thegem') .'">'.esc_attr_x('Search', 'submit button', 'thegem').'</button>
				</div>
			</form>';
	return $form;
}
...
function thegem_button($params = array(), $echo = false) {
	...
	if($params['tag'] === 'a' && (empty($params['href']) || $params['href'] === '#')) {
		$params['attributes']['role'] = 'button';
	}
	...
	if(!empty($post_item_data['show_featured_content']) || !$single) {
		if($format == 'video' && $post_item_data['video']) {
			...
			if($post_item_data['quote_author'] || !$single) {
				...
				$quote_link = !$single ? '<div class="quote-link"'.($post_item_data['quote_author_color'] ? ' style="color: '.$post_item_data['quote_author_color'].';"' : '').'><a href="'.esc_url(get_permalink($post_id)).'" aria-label="' . esc_attr__('Read More', 'thegem') . '"></a></div>' : '';
...
function thegem_single_post_template() {
	...
	$post_admin_data = thegem_get_sanitize_admin_post_data($post_id);
	if($post_admin_data['post_layout_settings'] == 'default') {
		$taxonomies = get_post_taxonomies($post_id);
		if(!empty($taxonomies)) {
			$post_terms = wp_get_object_terms($post_id, $taxonomies);
			if(!empty($post_terms) && is_array($post_terms)) {
				$posts_layout_data = false;
				foreach($post_terms as $term) {
					if(!empty($posts_layout_data)) continue;
					$term_data = thegem_get_output_cpt_archive_data($term->term_id, get_post_type($post_id));
					if(isset($term_data['posts_layout_settings']) && $term_data['posts_layout_settings'] === 'custom' && isset($term_data['posts_layout_source'])) {
						$post_data['post_layout_source'] = $term_data['posts_layout_source'];
						if(isset($term_data['posts_builder_template'])) {
							$post_data['post_builder_template'] = $term_data['posts_builder_template'];
						}
						$posts_layout_data = true;
					}
				}
			}
		}
...
function thegem_cpt_template() {
	...
	$cpt_admin_data = thegem_get_sanitize_admin_cpt_item_data($post_id);
	if($cpt_admin_data['layout_settings'] == 'default') {
		$taxonomies = get_post_taxonomies($post_id);
		if(!empty($taxonomies)) {
			$post_terms = wp_get_object_terms($post_id, $taxonomies);
			if(!empty($post_terms) && is_array($post_terms)) {
				...
				foreach($post_terms as $term) {
					...
					$term_data = thegem_get_output_cpt_archive_data($term->term_id, get_post_type($post_id));
					if(isset($term_data['posts_layout_settings']) && $term_data['posts_layout_settings'] === 'custom' && isset($term_data['posts_layout_source'])) {
						$cpt_data['layout_source'] = $term_data['posts_layout_source'];
						if(isset($term_data['posts_builder_template'])) {
							$cpt_data['builder_template'] = $term_data['posts_builder_template'];
						}
						$posts_layout_data = true;
					}
				}
...
function thegem_wpml_nav_menu_link_attributes_filter($atts, $menu_item, $args, $depth) {
	if('WPML_LS_Menu_Item' === get_class($menu_item)) {
		$atts['class'] .= ' thegem-wpml-ls-link';
	}
	return $atts;
}
add_filter('nav_menu_link_attributes', 'thegem_wpml_nav_menu_link_attributes_filter', 10, 4);

function thegem_wpml_object_id_filter_pll( $element_id, $element_type = 'post', $return_original_if_missing = false, $language_code = null ) {
	if(!is_admin() && defined('POLYLANG_VERSION') && !defined('ICL_SITEPRESS_VERSION') && $element_type === 'post') {
		$element_id = pll_get_post($element_id, $language_code);
	}
	return $element_id;
}
add_filter( 'wpml_object_id', 'thegem_wpml_object_id_filter_pll', 10, 4 );

add_filter('pll_copy_post_metas', 'thegem_pll_copy_post_metas');
function thegem_pll_copy_post_metas($metas) {
	return array_merge($metas, array('thegem_template_type'));
}
...
function theged_add_off_canvas_content_to_menu_start() {
	if(thegem_get_option('header_layout') == 'fullwidth_hamburger' || thegem_get_option('header_layout') == 'overlay') {
		if(thegem_get_option('header_content_offcanvas_area') == 'global_section' && thegem_get_option('header_content_offcanvas_global_section')) {
			$GLOBALS['thegem_offcanvas_menu_template'] = thegem_get_option('header_content_offcanvas_global_section');
...
function thegem_menu_insert_template($items, $args) {
	if(!empty($GLOBALS['thegem_offcanvas_menu_template']) && !isset($GLOBALS['thegem_mobile_menu_template'])) {
		$template_id = $GLOBALS['thegem_offcanvas_menu_template'];
		unset($GLOBALS['thegem_offcanvas_menu_template']);
