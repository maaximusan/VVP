(function($) {
	$(function() {
		...
		$form.on('click', '.support-links a', function(e) {
			e.preventDefault();
			var $link = $(this),
				$links = $link.closest('.support-links'),
				$paneModern = $links.nextAll('.support-pane.modern'),
				$paneLegacy = $links.nextAll('.support-pane.legacy');
			$('a', $links).removeClass('active');
			$paneModern.removeClass('active');
			$paneLegacy.removeClass('active');
			$link.addClass('active');
			if($link.is('.support-modern')) {
				$paneModern.addClass('active');
			} else {
				$paneLegacy.addClass('active');
			}
		});

		$form.on('click', '.combobox-input', function(e) {
			var $this = $(this);
			var $dropdown = $this.next('.combobox-dropdown');

			if ($dropdown.length === 0) {
				$dropdown = $('<div class="combobox-dropdown"></div>');
				$dropdown.insertAfter($this);

				var values = JSON.parse($this.attr('data-values'));

				$.each(values, function(key, value) {
					var $option = $('<div class="combobox-option"><input type="checkbox" value="' + key + '"><span>' + value + '</span></div>');
					$dropdown.append($option);
				});
			}

			var currentValues = $this.val().split(', ').filter(Boolean);
			$('.combobox-option input', $dropdown).each(function() {
				var $cb = $(this);
				var isChecked = currentValues.includes($cb.val());
				$cb.prop('checked', isChecked);
				$cb.closest('.combobox-option').toggleClass('selected', isChecked);
			});

			$dropdown.toggle();
		});

		$form.on('click', '.combobox-option', function(e) {
			var $checkbox = $(this).find('input');
			$checkbox.prop('checked', !$checkbox.prop('checked'));
			$(this).toggleClass('selected', $checkbox.prop('checked'));

			var selectedValues = [];
			$('.combobox-option', $(this).closest('.combobox-dropdown')).each(function() {
				var $cb = $(this).find('input');
				if ($cb.prop('checked')) {
					selectedValues.push($cb.val());
				}
			});
			$(this).closest('.combobox-container').find('.combobox-input').val(selectedValues.join(', '));
